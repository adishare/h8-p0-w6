...
function ganjilGenapAsianGames ( date, data ) {
  if(date<1 || date>31 || data.length<1) return 'Invalid dates'
  
  var plats = []
  for(var i=0;i<data.length;i++){
    if(data[i].type == 'Mobil') plats.push(Number(data[i].plat.match(/[0-9]/g).join('')))
  }
  var valueDateGanjil = true
  if(date%2 == 0) valueDateGanjil = false


  var result= 0
  for(var j=0;j<plats.length;j++){
    if(valueDateGanjil == true){
      if(plats[j]%2 == 0) result+=1
    } 
    else if(valueDateGanjil == false){
      if(plats[j]%2 != 0) result+=1
    }
  }
