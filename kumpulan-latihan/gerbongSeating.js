...
function trainSeating(penumpang) {
  ...
  for (var i = 0; i < penumpang.length; i++) {
    var objTempoPassenger = {
      nama: penumpang[i].nama,
      seat: penumpang[i].seat
    }

    // console.log(objTempoPassenger);

    if (penumpang[i].gerbong === "VVIP") {
      classObj.VVIP.push(objTempoPassenger)
    } else if (penumpang[i].gerbong === "VIP") {
      classObj.VIP.push(objTempoPassenger)
    } else if (penumpang[i].gerbong === "Regular") {
      classObj.Regular.push(objTempoPassenger)
    }
