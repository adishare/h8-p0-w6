...
function missingNum ( arr ) {
  numArr = arr.join(',').split(',')
  var deret = []
  for(var i=0;i<numArr.length;i++){
      if(numArr[i] != ' ' && arr.length>0) deret.push(Number(numArr[i]))
  }
  deret.sort(function(a,b){return a-b})
  var result = []

  for(var j=deret[0];j<deret[deret.length-1];j++){
      var bol = true
      for(var k=0;k<deret.length;k++){
          if(j == deret[k]) bol = false
      }
      if(bol == true) result.push(j)
  }
  return result
}
